...
- "conversion specification": Begin with the % character, its a placeholder representing a value to be filled in during printing. Information following the % character specifies how the value is converted from the binary form to printed form. %d tells that `printf` should convert an int value from binary to string of decimal digits.
...
- d: displays an integer in decimal form, p indicates minimum num of digits to display, extra zeros are added if needed
...
- g: displays a floating point number in either exponential or fixed decimal format, depending on the number's size. Unlike the f conversion, the g won't show trailing zeros. If the value to be printed has no digits after the decimal point, g doesn't display the decimal point.
...
and to print \, we can't just add itonce inside the strings as the compiler will assume its an escape sequence so use 
\\
```
...
The value that is read from the input won't be stored in the variable; instead, the variable will retain its old value(which may be meaningless if the variable wasn't given an initial value).
